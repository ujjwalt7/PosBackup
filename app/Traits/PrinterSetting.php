...
trait PrinterSetting
{
    ...
    {
        $orderNumber = __('modules.kot.order_number', ['number' => $order->order_number]);
        $kotNumber = __('modules.kot.kot_number', ['number' => $kot->kot_number]);
        $date = __('modules.kot.date', ['date' => $kot->created_at->format('d-m-Y')]);
        $time = __('modules.kot.time', ['time' => $kot->created_at->format('h:i A')]);
    ...
    {
        $amount = str_pad(currency_format($payment->amount, restaurant()->currency_id), 10);
