<div>
    ...
    <script>
        ...
        function initializePusher() {
            try {
                ...
                pusherChannel.bind('order.updated', function(data) {
                    console.log('🎉 Pusher: Order updated via Pusher:', data);
                    $wire.$refresh();
                });

                pusherChannel.bind('order.created', function(data) {
                    console.log('🎉 Pusher: Order created via Pusher:', data);
                    $wire.$refresh();
                });
        ...
        document.addEventListener('livewire:initialized', () => {
            if (typeof PUSHER_SETTINGS !== 'undefined' && PUSHER_SETTINGS.is_enabled_pusher_broadcast) {
                initializePusher();
            } else {
        ...
        $wire.watch('pollingInterval', (value) => {
            if (typeof PUSHER_SETTINGS !== 'undefined' && !PUSHER_SETTINGS.is_enabled_pusher_broadcast && $wire.get('pollingEnabled')) {
                startPolling();
            }
        });
